...
function extractImagesFromText(text: string): Array<{ data: string; type: string; original: string; isUrl: boolean }> {
  ...
  const possibleImageUrlPatterns = [
    /https?:\/\/antv-studio\.alipay\.com\/api\/gpt-vis\/[^\s<>"']+/gi,
...
function ensureFilenameExtension(filename: string, mimeType: string) {
  ...
  return safe + "." + extensionFromMimeType(mimeType);
}

function extractDownloadsFromText(text: string, preferredFilename?: string): ExtractedDownload[] {
  const downloads: ExtractedDownload[] = [];

  const dataUrlPattern = /data:([^;,\s]+)(?:;charset=[^;,\s]+)?;base64,([A-Za-z0-9+/=]+)/gi;
  let match;
  ...
  while ((match = dataUrlPattern.exec(text)) !== null) {
    ...
    const filename = ensureFilenameExtension(preferredFilename || "download", mimeType);
  ...
  return downloads.filter((d, index, self) => index === self.findIndex((x) => x.dataUrl === d.dataUrl));
}

function DownloadPreview({ downloads }: { downloads: ExtractedDownload[] }) {
    ...
    <div className="mt-3 space-y-2">
      {downloads.map((d, index) => (
        <a
          ...
          href={d.dataUrl}
          download={d.filename}
          className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-sm text-slate-700 transition-colors"
        >
          <span className="font-medium">{d.filename}</span>
          <span className="text-xs text-slate-500">{d.mimeType}</span>
        </a>
      ))}
    </div>
  );
    ...
    <>
      <div className="mt-3 space-y-3">
        {images.map((image, index) => {
          ...
          const filename = image.isUrl ? "chart.png" : `image-${index + 1}.png`;

          return (
            <div
              key={index}
              className="relative group border border-slate-200 rounded-lg overflow-hidden hover:border-slate-300 hover:shadow-sm transition-all duration-200 bg-white"
            >
              ...
              {loadedImages.has(index) && (
                <div className="absolute top-2 left-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <a
                    href={image.data}
                    className="bg-slate-800 bg-opacity-70 text-white text-xs px-2 py-1 rounded"
                    target="_blank"
                    rel="noreferrer"
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      window.open(image.data, "_blank", "noopener,noreferrer");
                    }}
                  >
                    打开
                  </a>
                  <button
                    ...
                    onClick={async (e) => {
                      ...
                      try {
                        const resp = await fetch(image.data, { mode: "cors" });
...
export const ToolCallBox = React.memo<ToolCallBoxProps>(({ toolCall, toolResult }) => {
  ...
  const { name, args, result, status, images, downloads } = useMemo(() => {
    if (!toolCall) {
      return {
        name: "Unknown Tool",
        args: {},
    ...
    const extractedDownloads = resultAsText ? extractDownloadsFromText(resultAsText, (toolArgs as any)?.filename) : [];

    const toolStatus = "completed";
  ...
  const hasContent = result || Object.keys(args).length > 0 || images.length > 0 || downloads.length > 0;
  ...
  return (
    <div className="w-full mb-3 group/toolbox">
      ...
      <DownloadPreview downloads={downloads} />
    </div>
  );
});
